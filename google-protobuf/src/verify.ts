...
function startRecordingServer(
  port: number,
  onUpdateInputs: (request: TestServiceUpdateInputsRequest.AsObject) => void
) {
  const server = new Server();

  server.addService(TestServiceService, {
    updateInputs: (call: any, callback: any) => {
      onUpdateInputs(call.request.toObject());
      callback(null, new TestServiceUpdateInputsResponse());
    },
  });
...
export async function verifyRoundTrip(port: number) {
  ...
  const response = (await readInputsCall(
    client
  )) as TestServiceReadInputsResponse.AsObject;
  const read = response.inputsList;
  ...
  let received: TestServiceUpdateInputsRequest.AsObject | undefined;
  ...
  assert.ok(received);
  const { userId, taxYear, formName, inputsList: updated } = received;
